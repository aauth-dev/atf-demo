...
say('the evaluator names it inside the signed appraisal. Withdrawal here is')
say('AAuth revocation, advertised as revocation_endpoint and shown below.')
...
say('')
say('The revoking call carries exp so the resource can size its store: an')
say('entry only has to outlive the token it names. AAuth’s revocation request')
say('has no exp field, which is spec issue #146. Without one this resource')
say('falls back to 24 hours.')
