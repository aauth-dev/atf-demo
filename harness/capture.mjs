...
say('')
say('Not shown here: atf_subject_mismatch, atf_appraisal_superseded and')
say('atf_status_unavailable, the three refusals no fresh token repairs. They')
say('need a contradicting claim, a published demotion event and a dead status')
say('channel respectively, so they are covered in test/gate.test.ts rather')
say('than by a client that only holds well-formed tokens.')
